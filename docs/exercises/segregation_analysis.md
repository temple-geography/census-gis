...
You will analyize segregation for counties in your state. This will include comparing two high population counties, and comparing a county to an earlier point in time. Begin by identifying the most populous counties in your state. Then pick the largest county, and one other interesting comparison county. I will use Pennsylvania counties in the example. The 2020 populations by race of the six largest counties in Pennsylvania are:
...
You can either download the two counties separately, or download all tracts for the state and then filter to get the counties you are analyzing. Either way, you should end up with two data frames, one for each county.
...
For this code to work, you need to also have a column named `geoid`. Your column may be `GEOID` (as originally downloaded), or you may have renamed it to something else.

After downloading data for Philadelphia and Allegheny counties, I have two data frames. The beginning of the data frame for Philadelphia looks like this:
...
We will calculate the Index of Dissimarity ($D$) for Philadelphia and Allegheny. $D$ is a measure of segregation between two groups, calculated as follows:
...
* The *p*-value is is $2.2 \times 10^{-16}$. This indicates a very, very small probability that there is no true difference between the means of the distributions. Another way of saying this is that it is highly, highly *unlikely* that segregation in Philadelphia and Allegheny is actually *the same*.
...
* The relevant table is P8 in 2000. Variable names have the same column number, but the template for the variable name is different. For example:
    ...
    * Non-Hispanic White population is `` in 2020 and `"P008003"` in 2000.
...
Often we are interested in quantifying and visualization *local* segregation. That is, for a small area *within* the region, such as a Census tract, we want to see how segregated that population is *in that area*. One way to do this is with a measure that shows how atypical the population is within the context of the larger region.

One way to do that is with the **location quotient**. We have already seen this measure when we discussed business datasets. It is frequently used as a measure of industrial concentration for a region, where it can tell us how concentrated a sector or industry is in comparison with the economy as a whole. Location quotients are also used in segregation research.

The location quotient is calculated as a ratio of ratios:

$$LQ = \frac{group/total}{GROUP/TOTAL}$$

where:

* $group$ is the group population in the small area
* $total$ is the total population in the small area
* $GROUP$ is the group population in the region
* $TOTAL$ is the total population in the region

We want to map the results, so begin by redownloading race data *with geometries* for 2020 the largest county in your state. You can also reuse the data frame that you created for Exercise 3b: Map-Making with Tmap v4.

Then add the location quotient to the data frame. Use one of the three largest race groups for your county. For Philadelphia, I calculated the location quotient for the Black population (the largest group in Philadelphia) as follows:
...
This map can be improved in one major way. Location quotients have a central value of 1, which means that a group's proportion in a Census tract is equal to their proportion in the region. Numbers above 1 mean the group is *concentrated* or *overrepresented* in that tract, and numbers below 1 mean the group is *underrepresented* in that tract. We would like to visually emphasize divergence from this central value. That is exactly what **diverging** color palettes are for.

Note, however, that in the legend the middle class is "1.0 to 1.5". We would like this middle class to go from slightly below 1 to slightly above 1. Keep in mind that the location quotient's minimum value is 0, but it's maximum value is infinity. We don't really want the middle class to run from 0.5 to 1.5, because 0.5 is in some sense *farther* below 1.0 than 1.5 is above it. This is because 0.5 represents *half* the "expected" ratio. On the other side of scale, we really want the value that represents *twice* the expected ratio, which is 2.0.
